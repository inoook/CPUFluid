	...
	*/	
	public void reset() {
		...
		for (int i = 0; i < _numCells; i++) {
			...
			r[i] = rOld[i] = g[i] = gOld[i] = b[i] = bOld[i] = 0.0f;
		}
	}
	...
	protected void advect(int b, float[] _d, float[] d0, float[] du, float[] dv) {
		...
		for (int i = 1; i <= _NX; i++) {
			for (int j = 1; j <= _NY; j++) {
				...
				if (x > _NX + 0.5f) x = _NX + 0.5f;
				if (x < 0.5f)     x = 0.5f;
				
				i0 = (int) x;
				i1 = i0 + 1;
				
				if (y > _NY + 0.5f) y = _NY + 0.5f;
				if (y < 0.5f)     y = 0.5f;
		...
		for (int k = 0; k < _solverIterations; k++) {		// MEMO
			for (int i = 1; i <= _NX; i++) {
				for (int j = 1; j <= _NY; j++) {
					...
					// HACK: inok 浮動小数点で少数値が増えすぎると、処理が重くなりframeRateが落ちるので、少数値を細かくしすぎないようにした
					// 本来であればbyteとかにした方がよいとおもう
					if(r[index5]< 0.00001f){
						r[index5] = 0;
					}
					if(g[index5]< 0.00001f){
						g[index5] = 0;
					}
					if(b[index5]< 0.00001f){
						b[index5] = 0;
					}
...
//	protected void DIFFUSE_UV()				{ diffuse(0, u, uOld, visc);	diffuse(0, v, vOld, visc); }
	...
	public bool enableBoundary = true;//inok
}
